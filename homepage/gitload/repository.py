"""
Classes dealing with repositories and their synchronization.
"""
...
class Repository:
    """Local repository that does not perform synchronization."""
...
class GitRepository(Repository):
    """Local git repository that can be synchronized with a remote git repository."""
    ...
    def update(self):
        ...
        self.git('fetch', '--depth=1', 'origin', '+refs/heads/*:refs/remotes/origin/*')
        self.git('stash', '-u')
    ...
    def initialize(self):
        ...
        self.git('clone', '--depth=1', '--branch', self.branch, self.url, '.')
